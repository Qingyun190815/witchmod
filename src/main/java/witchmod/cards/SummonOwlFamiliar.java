...
import witchmod.cards.familiar.OwlFamiliar;  // 引入猫随从的卡牌类
import witchmod.powers.SummonFamiliarPower;  // 引入召唤随从力量的类

/**
 * SummonOwlFamiliar（召唤猫头鹰随从）是一个与随从相关的卡牌：
 * - 效果：在你的回合开始时，将一只猫头鹰随从加入你的手牌。移除其他随从力量。
 * - 如果升级，猫头鹰随从将会是升级版。
 */
public class SummonOwlFamiliar extends AbstractWitchCard {
    // 卡牌的基础信息
    public static final String ID = "SummonOwlFamiliar";  // 唯一ID
    public static final String NAME = "Owl Familiar";     // 卡牌名称
    public static final String IMG = "cards/summonowl.png";  // 卡牌图片路径
    public static final String DESCRIPTION = "At the start of your turn, add an Owl to your hand. NL Removes other Familiar powers.";  // 描述：在你的回合开始时，将一只猫头鹰随从加入你的手牌。移除其他随从力量。
    public static final String DESCRIPTION_UPGRADED = "At the start of your turn, add an upgraded Owl to your hand. Removes other Familiar powers.";  // 升级描述：在你的回合开始时，将一只升级版猫头鹰随从加入你的手牌。移除其他随从力量。
    ...
    public SummonOwlFamiliar() {
        ...
        cardPreviewTooltip = new OwlFamiliar();  // 设置卡牌预览，显示猫头鹰随从的效果
    }
    ...
    /**
     * makeCopy 方法：创建卡牌的副本
     * @return 新的 SummonOwlFamiliar 实例
     */
    ...
    public void upgrade() {
        if (!upgraded) {  // 确保卡牌只被升级一次
            ...
            cardPreviewTooltip.upgrade();  // 升级卡牌预览效果（猫头鹰随从）
        }
    }
}
