...
import witchmod.cards.familiar.CatFamiliar;  // 引入猫随从的卡牌类
import witchmod.cards.familiar.FamiliarCardEnum;  // 引入随从卡的枚举
import witchmod.powers.SummonFamiliarPower;  // 引入召唤随从力量的类

/**
 * SummonCatFamiliar（召唤猫随从）是一个与随从相关的卡牌：
 * - 效果：在你的回合开始时，将一只猫随从加入你的手牌。移除其他随从力量。
 * - 如果升级，猫随从将会是升级版。
 */
public class SummonCatFamiliar extends AbstractWitchCard {
    // 卡牌的基础信息
    public static final String ID = "SummonCatFamiliar";  // 唯一ID
    public static final String NAME = "Cat Familiar";     // 卡牌名称
    public static final String IMG = "cards/summoncat.png";  // 卡牌图片路径
    public static final String DESCRIPTION = "At the start of your turn, add a Cat to your hand. NL Removes other Familiar powers.";  // 描述：在你的回合开始时，将一只猫随从加入你的手牌。移除其他随从力量。
    public static final String DESCRIPTION_UPGRADED = "At the start of your turn, add an upgraded Cat to your hand. Removes other Familiar powers.";  // 升级描述：在你的回合开始时，将一只升级版猫随从加入你的手牌。移除其他随从力量。
    ...
    public SummonCatFamiliar() {
        ...
        cardPreviewTooltip = new CatFamiliar();  // 设置卡牌预览，显示猫随从的效果
    }
    ...
    /**
     * makeCopy 方法：创建卡牌的副本
     * @return 新的 SummonCatFamiliar 实例
     */
    ...
    public void upgrade() {
        if (!upgraded) {  // 确保卡牌只被升级一次
            ...
            cardPreviewTooltip.upgrade();  // 升级卡牌预览效果（猫随从）
        }
    }
}
